/**
 * Criptografia AES-GCM para credenciais MP em repouso.
 * Chave: PAYMENT_CREDENTIALS_ENCRYPTION_KEY (mín. 16 caracteres; ideal 32+).
 */

function getEncryptionKeyBytes(): Uint8Array {
  const raw = (Deno.env.get('PAYMENT_CREDENTIALS_ENCRYPTION_KEY') ?? '').trim();
  if (raw.length < 16) {
    throw new Error('PAYMENT_CREDENTIALS_ENCRYPTION_KEY não configurada (mín. 16 caracteres).');
  }
