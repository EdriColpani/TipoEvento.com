...
serve(async (req) => {
  ...
  try {
    ...
    if (canManageErr) {
      console.error('[create-consumption-license-checkout] canManage:', canManageErr);
    ...
    const externalReference = `consumption_license_charge:${charge.id}`;
    const notificationUrl = `${Deno.env.get('SUPABASE_URL')}/functions/v1/mercadopago-webhook`;
    const successUrl = `${base}/manager/settings/company-profile?tab=billing&license_status=success&charge_id=${charge.id}`;
    const pendingUrl = `${base}/manager/settings/company-profile?tab=billing&license_status=pending&charge_id=${charge.id}`;
    const failureUrl = `${base}/manager/settings/company-profile?tab=billing&license_status=failure&charge_id=${charge.id}`;

    const preferenceData = {
      items: [
        {
          title: 'Licença mensal — consumo EventFest',
          unit_price: amount,
          quantity: 1,
          currency_id: 'BRL',
        },
    ...
    if (!mpRes.ok) {
      ...
      console.error('[create-consumption-license-checkout] MP error:', errText);
    ...
    await supabaseService.rpc('attach_consumption_license_charge_mp_preference', {
    ...
    console.error('[create-consumption-license-checkout]', e);
    return new Response(
