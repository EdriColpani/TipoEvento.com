-- Turmas por evento (limite de inscrições gratuitas por turma)

CREATE TABLE IF NOT EXISTS public.event_turmas (
...
CREATE INDEX IF NOT EXISTS idx_event_turmas_event_id ON public.event_turmas (event_id);
CREATE INDEX IF NOT EXISTS idx_event_turmas_created_by ON public.event_turmas (created_by);

ALTER TABLE public.event_registrations
  ADD COLUMN IF NOT EXISTS turma_id UUID REFERENCES public.event_turmas(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_event_registrations_event_turma ON public.event_registrations (event_id, turma_id);

-- RLS
ALTER TABLE public.event_turmas ENABLE ROW LEVEL SECURITY;

-- Public read for inscription screen
CREATE POLICY "event_turmas_select_public"
ON public.event_turmas
FOR SELECT
TO anon, authenticated
USING (true);

-- Only Admin Master (1) / Gestor PRO (2) can create/update turmas
CREATE POLICY "event_turmas_insert_admin_or_manager"
ON public.event_turmas
FOR INSERT
TO authenticated
WITH CHECK (
...
CREATE POLICY "event_turmas_update_admin_or_manager"
ON public.event_turmas
FOR UPDATE
TO authenticated
USING (
...
-- Helpers para inscrição por turma

-- Retorna turmas com vagas restantes
...
-- Atualiza a RPC de inscrição gratuita para respeitar capacidade por turma
-- Erros esperados:
-- - turma_full
-- - invalid_turma
...
BEGIN
  ...
  -- Regra: reserva 1 wristband_analytics com preço 0 do evento
