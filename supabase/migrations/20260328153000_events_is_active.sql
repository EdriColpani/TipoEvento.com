-- Eventos: flag de divulgação/vendas (desativar sem excluir)
ALTER TABLE public.events
  ADD COLUMN IF NOT EXISTS is_active boolean NOT NULL DEFAULT true;

COMMENT ON COLUMN public.events.is_active IS
  'Quando false, o evento não aparece na vitrine pública e não aceita novas compras/inscrições; validações de ingressos já emitidos podem continuar.';

-- RPC de inscrição gratuita: bloqueia evento desativado
