-- Bloqueia inscrição gratuita em eventos vitrine (listing_only)
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM events WHERE id = p_event_id AND COALESCE(listing_only, false) = true) THEN
    RETURN jsonb_build_object('ok', false, 'error', 'event_listing_only');
  END IF;
