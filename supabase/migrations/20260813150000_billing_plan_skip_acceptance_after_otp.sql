-- Fase 4: após OTP seguro, confirm/upgrade de plano não re-registra aceite (já gravado no finalize).

DROP FUNCTION IF EXISTS public.confirm_company_billing_plan(UUID, public.billing_plan_type, UUID);
DROP FUNCTION IF EXISTS public.confirm_company_billing_plan(UUID, public.billing_plan_type, UUID, TEXT, TEXT, BOOLEAN);

DROP FUNCTION IF EXISTS public.request_company_billing_plan_upgrade(UUID, public.billing_plan_type, UUID);
DROP FUNCTION IF EXISTS public.request_company_billing_plan_upgrade(UUID, public.billing_plan_type, UUID, TEXT, TEXT, BOOLEAN);
