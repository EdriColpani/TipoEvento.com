-- Planos com crédito (híbrido / licença) e empresas parceiras exigem PIX/banco
-- para o admin liquidar D+1. Mercado Pago sozinho não basta nesses planos.
-- O gate de evento e o aceite do contrato passam a exigir esses dados.

CREATE OR REPLACE FUNCTION public.company_plan_requires_bank_payout(
  p_plan TEXT,
  p_kind TEXT DEFAULT 'organizer'
)
RETURNS BOOLEAN
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT
    COALESCE(p_kind, 'organizer') = 'partner'
    OR COALESCE(p_plan, '') IN ('ticket_plus_consumption', 'consumption_or_license');
$$;

CREATE OR REPLACE FUNCTION public.company_has_complete_bank_payout(p_company_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.assert_company_bank_payout_for_billing_plan(
  p_company_id UUID,
  p_plan TEXT
)
RETURNS VOID
...
BEGIN
  ...
  IF p_company_id IS NULL OR COALESCE(trim(p_plan), '') = '' THEN
    RETURN;
  END IF;
  ...
  IF NOT public.company_plan_requires_bank_payout(p_plan, v_kind) THEN
    RETURN;
  END IF;

  IF NOT public.company_has_complete_bank_payout(p_company_id) THEN
    RAISE EXCEPTION
      'Antes de aceitar o contrato deste plano, cadastre conta bancária e chave PIX em Perfil da Empresa → Recebimento. Esses dados são obrigatórios para o repasse D+1 de crédito. Caminho: /manager/settings/company-profile?tab=payments';
  END IF;
END;
$$;
...
BEGIN
  ...
  -- Só divulgação: sem payout de ingresso/crédito
  IF v_plan = 'listing_monthly' AND v_kind <> 'partner' THEN
    RETURN true;
  END IF;
  ...
  v_bank_ok := v_has_profile AND public.company_payout_bank_fields_complete(
    v_row.bank_code, v_row.bank_name, v_row.agency, v_row.account_number,
    v_row.holder_name, v_row.holder_document, v_row.pix_key, v_row.pix_key_type
  );

  -- Parceira e planos com crédito: PIX/banco obrigatório (D+1)
  IF public.company_plan_requires_bank_payout(v_plan, v_kind) THEN
    IF NOT v_bank_ok THEN
      RETURN false;
    END IF;
    -- Híbrido com split MP: banco + MP conectado
    IF v_plan = 'ticket_plus_consumption' THEN
      v_mode := CASE WHEN v_has_profile THEN v_row.payout_mode ELSE NULL END;
      IF v_mode IS NULL OR v_mode = 'mercado_pago' THEN
  ...
  -- Organizador só ingresso: MP OU banco
  IF v_plan = 'ticket_commission' THEN
...
BEGIN
  ...
  IF NOT public.company_has_valid_payout_setup(p_company_id) THEN
    IF public.company_plan_requires_bank_payout(v_plan, v_kind) THEN
      RAISE EXCEPTION
        'Cadastro de eventos bloqueado: no plano com crédito, cadastre conta bancária e PIX em Perfil da Empresa → Recebimento (além do Mercado Pago, se o modo de ingresso for split). Caminho: /manager/settings/company-profile?tab=payments';
    END IF;
    RAISE EXCEPTION
...
BEGIN
  ...
  v_bank_required := public.company_plan_requires_bank_payout(v_plan, v_kind);
  v_bank_complete := public.company_has_complete_bank_payout(p_company_id);
...
BEGIN
  ...
  SELECT COALESCE(c.company_kind::text, 'organizer'), c.billing_plan::text
  INTO v_kind, v_plan
  FROM public.companies c
  WHERE c.id = p_company_id;
  ...
  IF v_mode = 'bank_transfer'
     OR public.company_plan_requires_bank_payout(v_plan, v_kind) THEN
    IF NOT public.company_payout_bank_fields_complete(
      ...
      RAISE EXCEPTION
        'Informe banco, agência, conta, titular, documento e chave PIX. No plano com crédito esses dados são obrigatórios para o repasse D+1.';
    END IF;
  END IF;
...
BEGIN
  ...
  PERFORM public.assert_company_bank_payout_for_billing_plan(p_company_id, p_plan::text);
...
BEGIN
  ...
  PERFORM public.assert_company_bank_payout_for_billing_plan(p_company_id, p_new_plan::text);
...
REVOKE ALL ON FUNCTION public.company_plan_requires_bank_payout(TEXT, TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.company_has_complete_bank_payout(UUID) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.assert_company_bank_payout_for_billing_plan(UUID, TEXT) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.company_plan_requires_bank_payout(TEXT, TEXT) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.company_has_complete_bank_payout(UUID) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.assert_company_bank_payout_for_billing_plan(UUID, TEXT) TO authenticated, service_role;
