-- =============================================================================
-- Recarga manual de crédito (teste / colaborador)
-- Cliente: edriads@gmail.com
-- Valor: R$ 10,00
--
-- Idempotente: se o mp_payment_id manual já existir como paid, não credita de novo.
-- Uso (SQL Editor / psql / MCP execute_sql):
--   \i supabase/scripts/grant_manual_credit_edriads_gmail.sql
-- =============================================================================

DO $$
DECLARE
  v_email TEXT := 'edriads@gmail.com';
  v_amount NUMERIC(12, 2) := 10.00;
  v_mp_payment_id TEXT := 'manual-restore:edriads@gmail.com:10.00:v1';
  v_user_id UUID;
  v_order_id UUID;
...
BEGIN
  ...
  INSERT INTO public.credit_topup_orders (
    ...
    'bank_transfer',
    'pix',
    format('Recarga manual R$ %s (teste colaborador)', to_char(v_amount, 'FM999999990.00')),
    'Script grant_manual_credit_edriads_gmail.sql — crédito de teste'
  )
...
WHERE lower(u.email) = 'edriads@gmail.com';
